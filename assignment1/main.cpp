 ...
 */
void write_courses_not_offered(std::string filename, std::vector<Course>& unlisted_courses) {
  std::ofstream ofs(filename);
  if(ofs.is_open()){
    ofs << "Title,Number of Units,Quarter" << "\n";
...
int main() {
  ...
  write_courses_offered(COURSES_OFFERED_PATH, courses);
  write_courses_not_offered(COURSES_NOT_OFFERED_PATH, courses);
